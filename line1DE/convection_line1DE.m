...
T_values = 273.15+[2,43.29,86.15,128.87,171.68,214.60];
...
t_totals = 20000; %200000 Total number of time steps saved
...
% mov = matfile('movie_1DE.mat','Writable',true);
...
% 
% plotint = 40; %Plotting interval
% if mod(count,plotint) == 1
%     figure(88)
%         set(gcf,'units','normalized','outerposition',[0.4 0.25 0.6 0.25])
...
%         caxis([T_s-273.15 200]) %max(T_bot_vec)-273.15])
%         title(c,'^{o}C')
%         title(sprintf('15 Ma - Temperature after %5d years',round(times(count-1)/(yrs2s))))
%         drawnow
...
% load movie_1DE.mat
% % F(1) = F(2);
% v = VideoWriter('convection_1DE.avi');
% open(v)
% writeVideo(v,F)
% close(v)