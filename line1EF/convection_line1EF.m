...
load phys_props.mat %Import physical property grids produced with 
...
T_values = 273.15+[2,57.98,115.47,172.82,230.31,287.40];
...
% mov = matfile('movie_1EF.mat','Writable',true);
...
% if mod(count,plotint) == 1
%     figure(88)
%         set(gcf,'units','normalized','outerposition',[0.4 0 0.6 0.25])
...
%         caxis([T_s-273.15 200]) %max(T_bot_vec)-273.15])
%         title(c,'^{o}C')
%         title(sprintf('7 Ma - Temperature after %5d years',round(times(count-1)/(yrs2s))))
%         drawnow
...
% load movie_1EF.mat
% % F(1) = F(2);
% v = VideoWriter('convection_1EF.avi');
% open(v)
% writeVideo(v,F)
% close(v)