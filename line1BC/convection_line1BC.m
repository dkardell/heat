...
T_values = 273.15+[2,27.04,53.78,80.38,106.92,133.50];
...
% mov = matfile('movie_1BC.mat','Writable',true);
...
figure(4)
set(gcf,'units','normalized','outerposition',[0.7 0 0.3 1]) %[0 0 1 0.9]
    subplot(5,1,1)
    plot(T_top)
...
%         caxis([T_s-273.15 200]) %max(T_bot_vec)-273.15])
%         title(c,'^{o}C')
%         title(sprintf('48 Ma - Temperature after %5d years',round(times(count-1)/(yrs2s))))
%         drawnow
...
% load movie_1BC.mat
% % F(1) = F(2);
% v = VideoWriter('convection_1BC.avi');
% open(v)
% writeVideo(v,F)
% close(v)