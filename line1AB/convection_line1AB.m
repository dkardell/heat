...
load phys_props.mat %Import physical properties
...
T_values = 273.15+[2,24.14,48.01,71.75,95.42,119.1];
...
Kbar               = comp_mean(kbar,1,kvkh,Grid); % arithmetic mean instead of harmonic mean
Rho_f              = rho_f;
KdMu               = Kd/Mu;
...
% mov = matfile('movie_1AB.mat','Writable',true);
...
%         caxis([T_s-273.15 200]) %max(T_bot_vec)-273.15])
%         title(c,'^{o}C')
%         title(sprintf('63 Ma - Temperature after %5d years',round(times(count-1)/(yrs2s))))
%         drawnow
...
% load movie_1AB.mat
% % F(1) = F(2);
% v = VideoWriter('convection_1AB.avi');
% open(v)
% writeVideo(v,F)
% close(v)